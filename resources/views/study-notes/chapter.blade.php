<x-study-notes>
    ...
    @section('keywords', $school->name . ',' . $exam->name . ',' . $topic->name)
    @section('canonical', config('app.url') . '/study-notes/' . $school->slug . '/' . $exam->slug . '/' . $topic->slug)
            ...
            <nav class="flex flex-col py-4 px-3 space-y-1">
                ...
                @foreach ($exams as $exam)
                    <details class="group [&_summary::-webkit-details-marker]:hidden"
                        @if (request()->route('exam')->name == $exam->name) open @endif>
                        <summary
                        ...
                        <nav class="mt-1.5 ml-4 flex flex-col border-l border-gray-200">
                            @foreach ($exam->topics as $topic)
                                <a href="{{ route('study-notes.content', ['school' => request('school'), 'exam' => $exam->slug, 'topic' => $topic->slug]) }}"
